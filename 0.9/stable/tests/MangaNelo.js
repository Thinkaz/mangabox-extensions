...
//#region node_modules/@paperback/types/lib/impl/SourceInfo.js
	...
	})(SourceIntents$1 || (SourceIntents$1 = {}));
	/**
	...
	*/
	...
	})(ContentRating$1 || (ContentRating$1 = {}));
...
//#endregion
//#region node_modules/@paperback/types/lib/compat/0.8/types.js
	var HomeSectionType;
	...
	*/
	...
	function convert08RequestTo09Request(request) {
		let url = request.url;
		if (request.param) url += request.param;
		const cookies = {};
		for (const cookie of request.cookies ?? []) cookies[cookie.name] = cookie.value;
		return {
			url,
	...
	function convert09RequestTo08Request(request) {
		return {
			url: request.url,
			method: request.method,
			headers: request.headers ?? {},
	...
	AppCompat.createRequestManager = function(info) {
		const interceptor = new class extends PaperbackInterceptor {
			constructor(legacyInterceptor) {
				super("main");
				_defineProperty(this, "legacyInterceptor", void 0);
			...
			async interceptRequest(request) {
				...
				return convert08RequestTo09Request(await this.legacyInterceptor.interceptRequest(oldRequest));
			}
			async interceptResponse(request, response, data) {
	...
	globalThis.App = new Proxy(AppCompat, { get(target, p) {
...
//#endregion
//#region node_modules/@paperback/types/lib/compat/0.8/searchFilters.js
	var SearchFilterForm = class extends AdvancedSearchForm {
		constructor(values, filters) {
			super();
			_defineProperty(this, "filters", void 0);
			_defineProperty(this, "selectedFilterValues", void 0);
			this.selectedFilterValues = {};
			for (const value of values ?? []) this.selectedFilterValues[value.id] = value.value;
			if (filters instanceof Promise) {
				this.filters = void 0;
				filters.then((filters) => this.filters = filters).catch((error) => this.filters = error).finally(() => this.reloadForm());
			} else this.filters = filters;
		}
		getSections() {
			if (!this.filters) return [Section("loading", [LabelRow("loading", { title: "Loading Filters" })])];
			else if (this.filters instanceof Error) return [Section("error", [LabelRow("error", {
				title: "Error loading search filters",
				subtitle: this.filters.message
			})])];
			else return this.filters.map((filter) => {
				switch (filter.type) {
					case "dropdown": {
						const selectedOptionId = [this.selectedFilterValues[filter.id] ?? filter.value];
					...
					case "multiselect": {
						const selectedOptions = this.selectedFilterValues[filter.id] ?? filter.value;
						return Section({ id: filter.id }, [TriStateSelectRow(filter.id, {
							title: filter.title,
							layout: "flow",
					...
					case "input": {
						const value = this.selectedFilterValues[filter.id] ?? filter.value;
						return Section({
							id: filter.id,
							header: filter.title
						}, [InputRow(filter.id, {
		...
		getSearchQueryMetadata() {
			if (this.filters && !(this.filters instanceof Error)) return this.filters.map((filter) => {
				return {
					id: filter.id,
		...
		async formDidSubmit() {
			if (!this.filters) throw new Error("Search filters are loading");
			else if (this.filters instanceof Error) throw this.filters;
		}
	};
...
//#region src/generic/network.ts
	var MangaboxInterceptor = class extends PaperbackInterceptor {
		...
		async interceptResponse(request, response, data) {
			...
			if (response.status !== 200) throw new Error(`Request failed with status ${response.status}: ${request.url}`);
			return data;
		}
	};
	...
	var MangaboxParser = class {
		constructor() {
			_defineProperty(this, "parseDate", (date) => {
				...
				if (date.includes("LESS THAN AN HOUR") || date.includes("JUST NOW")) return /* @__PURE__ */ new Date();
				if (date.includes("YESTERDAY")) return /* @__PURE__ */ new Date(Date.now() - 864e5);
				const timeUnits = {
					YEAR: 31556952e3,
					MONTH: 2592e6,
					WEEK: 6048e5,
			...
			_defineProperty(this, "isLastPage", ($) => {
		...
		async parseMangaDetails($, mangaId, source) {
			...
			const secondaryTitles = $(".story-alternative", context).first().text().trim().replace(/^Alternative\s*:\s*/i, "").split(";").map((title) => title.trim()).filter((title) => title.length > 0);
			const authors = $(".info-wrap > div:contains(\"Author(s):\")").first().clone().children("p").remove().end().text().trim();
			...
			const parsedStatus = $("li:contains(Status)", context).text().trim().toUpperCase();
			let status;
			...
			for (const obj of $("a", $("li.genres", context)).toArray()) {
				...
				if ([
					...
					"mature",
					"smut"
				].includes(title.toLowerCase())) contentRating = ContentRating$1.ADULT;
				genres.push({
					title,
					id
				});
		...
		parseChapterList(json, sourceManga, source) {
			const chapters = [];
			...
			} else console.log(`Invalid JSON structure for manga ${sourceManga.mangaId}`);
			return chapters;
		}
		async parseChapterDetails($, chapter, source) {
			const pages = [];
			for (const obj of $("img", "div.container-chapter-reader").toArray()) {
				const page = await this.getImageSrc($(obj), source);
				if (!page) {
					console.log(`Could not parse pages for mangaId:${chapter.sourceManga.mangaId} chapterId:${chapter.chapterId}`);
					continue;
				}
		...
		async parseDiscoverSections($, section, source) {
			const items = [];
		...
		async parseSearchResults($, source, query) {
			const results = [];
			if (query.title) {
		...
		async getImageSrc(imageObj, source) {
			...
			for (const attr of [
				...
				if (attr === "srcset") image = val.split(",")[0]?.trim().split(" ")[0] ?? "";
				else image = val;
				break;
			}
			if (image?.startsWith("/")) image = source.domain + image;
...
//#region src/generic/main.ts
	var Mangabox = class {
		constructor(params) {
			...
			_defineProperty(this, "defaultContentRating", void 0);
			_defineProperty(this, "language", void 0);
			_defineProperty(this, "parser", void 0);
			_defineProperty(this, "bypassPage", void 0);
			...
			this.defaultContentRating = params.contentRating ?? ContentRating$1.EVERYONE;
			...
			this.rateLimiter = params.rateLimiter ?? new BasicRateLimiter("ratelimiter", {
				numberOfRequests: 5,
				bufferInterval: 2,
				ignoreImages: true
			});
		...
		async getDiscoverSectionItems(section, metadata) {
			...
			switch (section.id) {
				...
				case "completed_titles":
					...
					break;
				default: throw new Error("Invalid sectionId provided!");
			}
			const [_response, buffer] = await Application.scheduleRequest({
		...
		async getSearchFilters() {
			...
			const $ = load(Application.arrayBufferToUTF8String(buffer));
			const genreTags = (await this.parser.parseSearchTags($)).find((x) => x.id === "genres");
			return [{
				type: "multiselect",
		...
		async saveCloudflareBypassCookies(cookies) {
			for (const cookie of cookies) this.cookieStorageInterceptor.deleteCookie(cookie);
		...
		constructSearchRequest(page, query) {
			...
			const urlBuilder = new URL$1(this.domain);
...
//#region src/generic/config.ts
	...
	const basePbConfig = {
		...
		icon: "icon.png",
		language: "en",
		contentRating: ContentRating$1.EVERYONE,
		capabilities: [
			SourceIntents$1.CHAPTER_PROVIDING,
			SourceIntents$1.DISCOVER_SECTION_PROVIDING,
			SourceIntents$1.SETTINGS_FORM_PROVIDING,
			SourceIntents$1.SEARCH_RESULT_PROVIDING,
			SourceIntents$1.CLOUDFLARE_BYPASS_PROVIDING
		],
		badges: [],
		developers: [{
			name: "Inkdex",
...
//#region src/MangaNelo/main.ts
	...
	const MangaNelo = new MangaNeloExtension();
...
//#region src/tests/suite.ts
	...
	const registerDefaultTests = function(suite, extension, extensionInfo, testData = {}) {
		...
		if (sourceCapabilities & SourceIntents$1.SEARCH_RESULT_PROVIDING && testData.searchResultsProviding !== false) if (implementsSearchResultsProviding(extension)) registerDefaultSearchResultsProvidingSourceTests(suite, extension, testData.searchResultsProviding);
		...
		if (sourceCapabilities & SourceIntents$1.CHAPTER_PROVIDING && testData.chapterProviding !== false) if (implementsChapterProviding(extension)) registerDefaultChapterProvidingSourceTests(suite, extension, testData.chapterProviding);
...
//# sourceMappingURL=MangaNelo.js.map